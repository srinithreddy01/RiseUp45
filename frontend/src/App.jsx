...
import {
  ...
  ChevronRight, Clock3, Code2, Dumbbell, Ellipsis, Flame, Home,
...
const weeklyGymKey = date => `weekly-${date.toLocaleDateString('en-US', { weekday: 'long' })}`
function migrateGymExercisesToWeekly(exercises) {
  if (!exercises || typeof exercises !== 'object') return exercises
  const next = { ...exercises }
  let changed = false
  Object.entries(exercises)
    .filter(([key, value]) => /^\d{4}-\d{2}-\d{2}$/.test(key) && Array.isArray(value) && value.length)
    .sort(([a], [b]) => b.localeCompare(a))
    .forEach(([key, value]) => {
      const weeklyKey = weeklyGymKey(new Date(`${key}T12:00:00`))
      if (!Array.isArray(next[weeklyKey])) { next[weeklyKey] = value; changed = true }
    })
...
function Workspace({ user, users, updateUser, onSwitchUser }) {
  ...
  const [gymExercises, setGymExercises] = useUserStoredState(user, 'gym-exercises', {})
  ...
  useEffect(() => { setGymExercises(migrateGymExercisesToWeekly) }, [setGymExercises])
...
useEffect(() => {
  if (!settings.alarm && (!settings.notifications || !('Notification' in window))) return
  ...
  const check = async () => {
    ...
    for (const task of tasks.filter(
      ...
      // -----------------------------
      if (settings.notifications && ('Notification' in window) && Notification.permission === 'granted' && currentMinutes === taskMinutes - 1) {
        const key = `${todayKey}-${task.id}-before`
      ...
      if (currentMinutes >= taskMinutes) {
        const key = `${todayKey}-${task.id}-alarm`

        if (reminded.current.has(key)) continue

        reminded.current.add(key)

        // Play your existing RiseUP alarm
        playAlarm()

        if (!settings.notifications || !('Notification' in window) || Notification.permission !== 'granted') continue
  ...
  useEffect(() => {
    ...
    const checkGymAlarm = async () => {
      ...
      const options = { body: 'It is 4:30 PM — time to get ready for your workout.', tag: `gym-alarm-${key}`, data: { type: 'gym-alarm' } }
      try { const registration = await navigator.serviceWorker?.ready; if (registration) await registration.showNotification('🏋️ Time to go GYM!', options); else new Notification('🏋️ Time to go GYM!', options) } catch { new Notification('🏋️ Time to go GYM!', { body: options.body }) }
    }
  ...
  const todayGymKey = weeklyGymKey(new Date())
  // Exercises are a weekly plan. Date-keyed entries are retained as a fallback
  // for workouts saved before weekly repetition was introduced.
  const todayGymExercises = Array.isArray(gymExercises[todayGymKey]) ? gymExercises[todayGymKey] : (Array.isArray(gymExercises[today]) ? gymExercises[today] : [])
  ...
  const addGymExercise = exercise => {
    const name = exercise.name.trim()
    const sets = Number(exercise.sets)
    const reps = Number(exercise.reps)
    if (!name || !Number.isInteger(sets) || sets < 1 || !Number.isInteger(reps) || reps < 1) return false
    setGymExercises(old => ({ ...old, [todayGymKey]: [...(Array.isArray(old[todayGymKey]) ? old[todayGymKey] : []), { id: makeId(), name, sets, reps }] }))
    setGym(old => ({ ...old, [today]: false }))
    notify(`${name} added to every ${workout.day} workout`)
    return true
  }
  const removeGymExercise = id => setGymExercises(old => ({ ...old, [todayGymKey]: (Array.isArray(old[todayGymKey]) ? old[todayGymKey] : []).filter(exercise => exercise.id !== id) }))
  const toggleGym = () => { setGym(old => ({ ...old, [today]: !old[today] })); notify(gymDone ? 'Workout moved back to pending' : 'Workout complete — great work!') }
  const selectPage = value => { setPage(value); window.scrollTo({ top: 0, behavior: 'smooth' }) }

  const shared = { tasks, setTasks, gym, setGym, gymExercises, todayGymExercises, addGymExercise, removeGymExercise, dailyHistory, todayTasks, completed, percent, streak, week, workout, gymDone, learning, setLearning, todayLearn, toggleTask, removeTask, setForm, setMenuTask, menuTask, toggleGym, updateLearning, saveLearningCard, deleteLearningCard, notify, settings, setSettings, selectPage, playAlarm }
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span className="brand-mark"><img src="/riseup45-logo.jpeg" alt="RiseUp 45"/></span><span>RiseUP</span></div>
...
function GymPage(p) {
  ...
  const selected = workoutForDate(new Date())
  const selectedDone = Boolean(p.gym[dateKey()])
  const sets = p.todayGymExercises.flatMap(exercise => Array.from({ length: exercise.sets }, (_, index) => ({ exercise, set: index + 1 })))
  return <><section className="gym-hero"><div><span className="section-icon purple big"><Dumbbell size={28}/></span><p className="eyebrow">WEEKLY TRAINING PLAN</p><h2>{selected.day} — {selected.name}</h2><p>{selected.rest ? 'A rest day keeps you ready for your next session.' : 'Add your exercises, then swipe right when the session is complete.'}</p></div><div className="workout-status"><span>{selectedDone ? 'DONE' : 'SCHEDULED'}</span><strong>{selectedDone ? 'Workout completed' : 'Ready to train'}</strong></div></section><section className="gym-layout"><div className="panel workout-detail"><div className="exercise-title"><div><p className="eyebrow">TODAY'S SESSION</p><h2>{selected.name}</h2></div>{!selected.rest && <button className="secondary-button add-exercise" onClick={() => setAddingExercise(true)}><Plus size={16}/> Add exercise</button>}</div>{selected.rest ? <Empty text="Rest, recover, and get ready for the week ahead."/> : <>{sets.length ? <div className="exercise-set-list">{sets.map(({ exercise, set }) => <div className="exercise-row" key={`${exercise.id}-${set}`}><span>{set}</span><div><strong>{exercise.name}</strong><small>Set {set} of {exercise.sets} · {exercise.reps} reps</small></div>{set === 1 && <button className="delete-exercise" onClick={() => p.removeGymExercise(exercise.id)} aria-label={`Remove ${exercise.name}`}><Trash2 size={17}/></button>}</div>)}</div> : <div className="gym-empty"><Dumbbell size={19}/><span>Add an exercise to begin. Each set will appear as its own workout row.</span></div>}<WorkoutCompletionSlide completed={selectedDone} onComplete={p.toggleGym}/></>}</div><div className="panel weekly-plan"><p className="eyebrow">YOUR SPLIT</p>{['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].map((day, index) => { const w = workoutForDate(new Date(2024, 0, index + 1)); return <div className={day === selected.day ? 'split-row active' : 'split-row'} key={day}><span>{day.slice(0,3)}</span><strong>{w.name}</strong>{w.rest ? <span>Rest</span> : <Dumbbell size={16}/>}</div> })}</div></section>{addingExercise && <GymExerciseForm onClose={() => setAddingExercise(false)} onSave={exercise => { if (p.addGymExercise(exercise)) setAddingExercise(false) }}/>}</>
}

function WorkoutCompletionSlide({ completed, onComplete }) {
  const track = useRef(null)
  const [progress, setProgress] = useState(completed ? 1 : 0)
  const dragging = useRef(false)
  useEffect(() => setProgress(completed ? 1 : 0), [completed])
  const updateProgress = event => {
    ...
    setProgress(next)
    if (next >= .86) { dragging.current = false; setProgress(1); onComplete() }
  }
  const finish = () => { if (!completed) setProgress(0); dragging.current = false }
  const completeWithKeyboard = event => { if (completed || !['Enter', ' '].includes(event.key)) return; event.preventDefault(); onComplete() }
  return <div ref={track} className={`workout-slide ${completed ? 'complete' : ''}`} role="slider" aria-label={completed ? 'Workout completed' : 'Slide right to complete workout'} aria-valuemin="0" aria-valuemax="100" aria-valuenow={Math.round(progress * 100)} aria-valuetext={completed ? 'Workout completed' : 'Slide right to complete workout'} tabIndex={0} onKeyDown={completeWithKeyboard} onPointerMove={updateProgress} onPointerUp={finish} onPointerCancel={finish}><span className="slide-label">{completed ? 'Workout completed' : 'Slide right to complete'}</span><button type="button" className="slide-knob" style={{ left: `calc(${progress * 100}% + ${(1 - progress) * 3}px)` }} onPointerDown={event => { if (completed) return; dragging.current = true; event.currentTarget.setPointerCapture(event.pointerId); updateProgress(event) }} aria-hidden="true">{completed ? <Check size={18}/> : <ChevronRight size={19}/>}</button></div>
}

function GymExerciseForm({ onClose, onSave }) {
  const [draft, setDraft] = useState({ name: '', sets: '', reps: '' })
  const update = (key, value) => setDraft(current => ({ ...current, [key]: value }))
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="task-modal gym-exercise-form" onMouseDown={event => event.stopPropagation()} onSubmit={event => { event.preventDefault(); onSave(draft) }}><div className="modal-header"><div><p className="eyebrow">ADD EXERCISE</p><h2>Build your workout</h2></div><button type="button" className="icon-button" onClick={onClose}><X/></button></div><label className="input-label">Workout name<input autoFocus required placeholder="e.g. Bench press" value={draft.name} onChange={event => update('name', event.target.value)}/></label><label className="input-label">Number of sets<input required type="number" min="1" max="99" inputMode="numeric" placeholder="e.g. 3" value={draft.sets} onChange={event => update('sets', event.target.value)}/></label><label className="input-label">Reps per set<input required type="number" min="1" max="999" inputMode="numeric" placeholder="e.g. 10" value={draft.reps} onChange={event => update('reps', event.target.value)}/></label><p className="form-hint">For 3 sets and 10 reps, this exercise will show three rows with 10 reps each.</p><div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit"><Plus size={17}/> Add exercise</button></div></form></div>
}
