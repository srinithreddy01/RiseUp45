...
export 'utils/date_time.dart' show nextOccurrence;

void main() {
  WidgetsFlutterBinding.ensureInitialized();
  runApp(const RiseUpWebApp());
}

class RiseUpWebApp extends StatefulWidget {
  const RiseUpWebApp({super.key});

  @override
  State<RiseUpWebApp> createState() => _RiseUpWebAppState();
}

class _RiseUpWebAppState extends State<RiseUpWebApp> {
  ...
  void initState() {
    ...
    _controller = WebViewController()
      ...
      ..loadFlutterAsset('assets/web/app.html');
  }
...
class _RiseUpAppState extends State<RiseUpApp> {
  ...
  void _setSession(AuthSession session) => setState(() => _session = session);
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        navigationBarTheme: const NavigationBarThemeData(
          ...
          elevation: 0,
          indicatorColor: Color(0x263F6DE0),
        ),
      ),
      darkTheme: ThemeData(
        colorScheme:
...
class _RiseUpHomeState extends State<RiseUpHome> with WidgetsBindingObserver {
  ...
  Future<void> _load() async {
    ...
    if (raw != null) {
      try {
        ...
        _gymPlans = ((data['gymPlans'] as List?) ?? [])
    ...
    await _consumeCompletedAlarmActions(refreshUi: false);
    if (mounted) {
      widget.onThemeChanged(_mode);
      setState(() => _loaded = true);
      await AlarmService.instance.syncAll(_tasks);
      await _syncGymPlans();
      await _refreshReminderState();
    }
  }

  Future<void> _consumeCompletedAlarmActions({bool refreshUi = true}) async {
    final completedIds = await AlarmService.instance.consumeCompletedTaskIds();
    if (completedIds.isEmpty) return;
    final completed = completedIds.toSet();
    _tasks = _tasks
        .map((task) =>
            completed.contains(task.id) ? task.copyWith(done: true) : task)
        .toList();
    await _save();
  ...
  Future<void> _refreshReminderState({bool resync = false}) async {
    final state = await AlarmService.instance.permissionState();
    if (resync) await AlarmService.instance.syncAll(_tasks);
  ...
  Future<void> _save() async {
    ...
    await prefs.setString(
        ...
        jsonEncode({
          ...
          'gym': _gym,
          'gymPlans': _gymPlans.map((plan) => plan.toJson()).toList(),
          'theme': _mode.name,
        }));
  }
  ...
  Future<ReminderScheduleResult> _upsertTask(Task task) async {
    ...
    await _save();
    return AlarmService.instance.schedule(task);
  }
  ...
  void _deleteTask(Task task) {
    ...
    _save();
    AlarmService.instance.cancel(task.id);
  }
  ...
  Task _gymReminderTask(GymPlan plan, int weekday) => Task(
        id: 'gym-${plan.id}-$weekday',
        title: 'Gym: ${plan.title}',
        category: 'Fitness',
        priority: 'High',
        date: _nextDateForWeekday(weekday),
        time: plan.time,
        done: false,
        recurrence: TaskRecurrence.weekly,
      );

  Future<void> _cancelGymPlanAlarms(String planId) async {
    for (var weekday = DateTime.monday; weekday <= DateTime.sunday; weekday++) {
      await AlarmService.instance.cancel('gym-$planId-$weekday');
    }
  }

  Future<void> _syncGymPlans() async {
    for (final plan in _gymPlans) {
      for (final weekday in plan.weekdays) {
        await AlarmService.instance.schedule(_gymReminderTask(plan, weekday));
      }
    }
  }

  Future<void> _upsertGymPlan(GymPlan plan) async {
    await _cancelGymPlanAlarms(plan.id);
    setState(() {
      final index = _gymPlans.indexWhere((item) => item.id == plan.id);
      if (index == -1) {
        _gymPlans.add(plan);
      } else {
        _gymPlans[index] = plan;
      }
    });
    await _save();
    for (final weekday in plan.weekdays) {
  ...
  Future<void> _deleteGymPlan(GymPlan plan) async {
    setState(() => _gymPlans.removeWhere((item) => item.id == plan.id));
    await _cancelGymPlanAlarms(plan.id);
    await _save();
  }

  Future<void> _editGymPlan([GymPlan? plan]) async {
    final value = await showModalBottomSheet<GymPlan>(
      context: context,
      isScrollControlled: true,
      showDragHandle: true,
      builder: (_) => GymPlanEditor(plan: plan),
    );
    if (value == null) return;
    await AlarmService.instance.requestNotificationPermission();
    await _upsertGymPlan(value);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${value.title} reminders are scheduled.')),
    );
  }
  ...
  Future<void> _enableReminders() async {
    ...
    if (!notificationsAllowed) {
      await AlarmService.instance.openNotificationSettings();
      await _refreshReminderState();
      if (mounted) {
  ...
  Future<void> _testReminder() async {
    final shown = await AlarmService.instance.showTestNotification();
    await _refreshReminderState();
    if (!mounted) return;
  ...
  Widget _buildPage() {
    switch (_page) {
      ...
      case 2:
        return GymPage(
          plans: _gymPlans,
          done: _gym[dateKey(DateTime.now())] ?? false,
          onToggle: _toggleGym,
          onAdd: () => _editGymPlan(),
          onEdit: _editGymPlan,
          onDelete: _deleteGymPlan,
        );
      case 3:
        return LearningPage(
            goals: _goals,
...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final scheduledWorkout = workoutFor(DateTime.now());
    final todayPlans = plans
        .where((plan) => plan.weekdays.contains(DateTime.now().weekday))
        .toList();
    final workout = todayPlans.isEmpty
        ? scheduledWorkout
        : Workout(
            scheduledWorkout.day,
            todayPlans.map((plan) => plan.title).join(' + '),
            todayPlans.expand((plan) => plan.exercises).toList(),
          );
    return PageFrame(
...
class GymPage extends StatelessWidget {
  ...
  final Future<void> Function([GymPlan? plan]) onEdit;
  final Future<void> Function(GymPlan plan) onDelete;
  ...
  Widget build(BuildContext context) {
    ...
    return PageFrame(
      title: 'Your gym plan',
      subtitle: 'Workouts repeat every selected weekday.',
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                if (todayPlans.isEmpty)
                  const Text(
                      'Create a plan and choose the days and time that work for you.')
                else
                  ...todayPlans.map(
                    (plan) => Padding(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('${plan.time}  ·  alert 5 minutes before'),
                              ],
                            ),
                          ),
                          IconButton(
                ...
                if (todayPlans.isNotEmpty)
                  FilledButton.icon(
                    ...
                    label: Text(done ? 'Workout completed' : 'Start workout'),
                  ),
              ],
            ),
          ),
          ...
          if (plans.isEmpty)
            GlassCard(
              child: Column(
                children: [
                  const Icon(Icons.add_task_rounded, size: 38),
                  const SizedBox(height: 10),
                  const Text('Your routine starts here.'),
                  const SizedBox(height: 10),
                  OutlinedButton(
                    onPressed: onAdd,
                ...
                child: GlassCard(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(
                      backgroundColor: const Color(0xff7c4dff).withOpacity(.16),
                    ...
                    subtitle: Text(
                      '${_daySummary(plan.weekdays)}  ·  ${plan.time}\n${plan.exercises.length} exercise${plan.exercises.length == 1 ? '' : 's'}',
                    ),
                    isThreeLine: true,
                    trailing: PopupMenuButton<String>(
                      onSelected: (value) {
                        if (value == 'edit') onEdit(plan);
                        if (value == 'delete') onDelete(plan);
                      },
                      itemBuilder: (_) => const [
                        PopupMenuItem(value: 'edit', child: Text('Edit plan')),
                        PopupMenuItem(
                            value: 'delete', child: Text('Delete plan')),
                      ],
                    ),
                    onTap: () => onEdit(plan),
                  ),
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  @override
  Widget build(BuildContext context) => GlassBackground(
        child: Scaffold(
          backgroundColor: Colors.transparent,
          body: SafeArea(
            child: Center(
                  ...
                  child: GlassCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          _register ? 'Create your space' : 'Welcome back',
                          style: Theme.of(context)
                              .textTheme
                              .headlineMedium
                              ?.copyWith(
                        ...
                        Text(
                          _register
                              ? 'Plan tasks, build routines, and make progress.'
                              : 'Sign in to continue with your daily plan.',
                        ),
                        const SizedBox(height: 24),
                        if (_register) ...[
                          TextField(
                        ...
                        const SizedBox(height: 4),
                        Text(
                          'This local account stays on this device.',
        ...
        child: Stack(
          children: [
            Positioned(
              ...
              right: -80,
              child: _GlowOrb(
                  color: const Color(0xff8ba8ff).withOpacity(.42), size: 260),
            ),
            Positioned(
              bottom: 40,
              left: -100,
              child: _GlowOrb(
                  color: const Color(0xffd6a6ff).withOpacity(.32), size: 250),
            ),
            child,
          ],
        ),
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(30),
            border: Border.all(
              color: (accent ?? Colors.white).withOpacity(isDark ? .20 : .70),
            ),
            boxShadow: [
              BoxShadow(
...
class GymPlanEditor extends StatefulWidget {
  const GymPlanEditor({super.key, this.plan});
  final GymPlan? plan;

  @override
  State<GymPlanEditor> createState() => _GymPlanEditorState();
}

class _GymPlanEditorState extends State<GymPlanEditor> {
  late final TextEditingController _title =
      TextEditingController(text: widget.plan?.title ?? '');
  final _exercise = TextEditingController();
  late Set<int> _weekdays =
      (widget.plan?.weekdays ?? [DateTime.now().weekday]).toSet();
  late TimeOfDay _time = widget.plan == null
      ? const TimeOfDay(hour: 18, minute: 30)
      : timeFrom(widget.plan!.time);
  late List<String> _exercises =
      List<String>.from(widget.plan?.exercises ?? []);
  ...
  void _addExercise() {
    final value = _exercise.text.trim();
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 6),
                const Text(
                    'Pick your routine and days. Your workout repeats every selected weekday.'),
                const SizedBox(height: 20),
                TextField(
                  controller: _title,
                  textCapitalization: TextCapitalization.words,
                ...
                Wrap(
                  ...
                  children: const [
                    ...
                    'Sun'
                      ...
                      .map(
                        (entry) =>
                            _DayChip(day: entry.key + 1, label: entry.value),
                      )
                      .toList(),
                ),
                const SizedBox(height: 20),
                GlassCard(
                  ...
                  child: ListTile(
                    contentPadding: EdgeInsets.zero,
                    leading:
                        const CircleAvatar(child: Icon(Icons.alarm_rounded)),
                    title: const Text('Gym time'),
                    subtitle:
                        const Text('You will get an alert 5 minutes before.'),
                    trailing: TextButton(
                      onPressed: () async {
                        final value = await showTimePicker(
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: () {
                      if (_title.text.trim().isEmpty || _weekdays.isEmpty)
                        return;
                      Navigator.pop(
                        context,
                        GymPlan(
                    ...
                    label: const Text('Save plan & reminders'),
                  ),
                ),
              ],
            ),
...
class _DayChipState extends State<_DayChip> {
  @override
  Widget build(BuildContext context) {
    final editor = context.findAncestorStateOfType<_GymPlanEditorState>();
    final selected = editor?._weekdays.contains(widget.day) ?? false;
    return FilterChip(
      label: Text(widget.label),
      selected: selected,
      onSelected: (value) => editor?.setState(() {
        if (value) {
          editor._weekdays.add(widget.day);
        } else {
          editor._weekdays.remove(widget.day);
        }
      }),
    );
  }
